...
const channels = [
  {
    ...
    cta: "Start Chat",
    link: "https://api.whatsapp.com/send/?phone=919600921734", // WhatsApp number for direct chat
  },
  {
    icon: Mail,
    title: "Email Us",
    desc: "Drop us an email anytime",
    info: "info@upzypencilbitz.com",
  },
  {
    icon: Phone,
    title: "Call Us",
    ...
    <section className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-24 sm:pt-32 pb-16 md:pb-24 lg:pb-32">
      ...
      <div className="grid lg:grid-cols-2 gap-12 lg:gap-16 xl:gap-24 items-center">
        <div className="text-left order-2 lg:order-1">
          <span className="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full bg-[#FF7A00]/10   border border-[#FF7A00]/20 text-[#FF7A00] text-[10px] sm:text-xs font-black uppercase tracking-[0.2em] mb-6 sm:mb-8">
          ...
          <p className="text-base sm:text-lg text-zinc-400 max-w-xl mb-10 sm:mb-12 leading-relaxed font-medium">
            Have a project in mind or just want to say hi? We're here to help
            you navigate your learning journey.
          </p>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-1 gap-4">
            {features.map(({ icon: Icon, title, desc }) => (
              ...
              >
                ...
                <div>
                  <h3 className="font-bold text-sm sm:text-base text-white mb-0.5 sm:mb-1">
                    {title}
                  </h3>
  ...
  return (
    ...
    >
      <input
        type="hidden"
        name="access_key"
        value="38dc468e-933e-48f1-8d9f-927c78029e24"
      />
      <input
        type="hidden"
        name="subject"
...
function Field({ icon: Icon, ...props }) {
  return (
    <div className="relative group w-full">
      <Icon className="absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 text-zinc-500 group-focus-within:text-[#FF7A00] transition-colors" />
      <input
        {...props}
        className="w-full h-12 sm:h-14 bg-black/50 border border-white/10 rounded-2xl pl-12 pr-4 text-sm focus:outline-none focus:border-[#FF7A00]/50 focus:ring-4 focus:ring-[#FF7A00]/5 transition-all placeholder:text-zinc-600"
      />
    </div>
  );
}
  ...
  return (
    <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16 sm:py-24">
      <div className="text-center mb-12 sm:mb-16">
        ...
        <div className="w-16 sm:w-20 h-1.5 bg-[#FF7A00] mx-auto rounded-full"></div>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 sm:gap-8">
        {channels.map(({ icon: Icon, title, desc, info, cta }) => (
          <div
            key={title}
            className="group bg-[#16161D] border border-white/5 rounded-[1.75rem] sm:rounded-[2rem] p-6 sm:p-8 text-center hover:border-[#FF7A00]/30 transition-all duration-500 lg:hover:-translate-y-2 flex flex-col justify-between"
          >
            <div>
              <div className="w-14 h-14 sm:w-16 sm:h-16 mx-auto mb-5 sm:mb-6 rounded-2xl bg-[#FF7A00]/10 flex items-center justify-center text-[#FF7A00] group-hover:scale-110 group-hover:bg-[#FF7A00] group-hover:text-white transition-all duration-500">
                <Icon className="w-6 h-6 sm:w-7 h-7" />
              </div>
              <a href={channels.link} target="_blank" rel="noopener noreferrer">
                <h3 className="text-lg sm:text-xl font-bold mb-2 sm:mb-3">
                  {title}
                </h3>
              </a>
              <p className="text-zinc-500 text-xs sm:text-sm mb-4 leading-relaxed max-w-xs mx-auto">
                {desc}
              </p>
            </div>
            ...
            <div>
              ...
              <p className="text-white font-bold text-xs sm:text-sm mb-5 break-words px-2">
                {info}
              </p>

              {cta ? (
                <a
                  href="https://api.whatsapp.com/send/?phone=919600921734"
                  target="_blank"
                  rel="noopener noreferrer"
                  className="inline-flex items-center gap-2 text-[#FF7A00] font-black text-[10px] sm:text-xs uppercase tracking-widest group-hover:gap-4 transition-all"
                >
                  Start Chat
      ...
      <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-6 mb-12 sm:mb-16">
        <div className="max-w-2xl">
          <h2 className="text-3xl sm:text-4xl font-black mb-3 sm:mb-4 tracking-tight">
            Still Have <span className="text-[#FF7A00]">Questions?</span>
          </h2>
        ...
        <button className="w-full sm:w-auto px-5 py-3 shrink-0 bg-white/5 border border-white/10 rounded-xl text-[#FF7A00] font-bold text-sm hover:bg-white/10 transition-all flex items-center justify-center gap-2">
          View All FAQs
        ...
        {faqs.map((faq, index) => {
          ...
          return (
            ...
            >
              ...
              >
                <span
                  className={`font-bold text-sm sm:text-base transition-colors ${isOpen ? "text-[#FF7A00]" : "text-white"}`}
                >
                  {faq.q}
                </span>

                <div
                  className={`w-7 h-7 sm:w-8 sm:h-8 rounded-lg flex items-center justify-center shrink-0 transition-all ${
                    isOpen
                      ? "bg-[#FF7A00] text-[#0B0B0F] rotate-180"
